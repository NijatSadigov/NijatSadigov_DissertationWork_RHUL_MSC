...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Logic:} Rather than seeking out a solution, this method takes an entire Boolean formula and creates a unique, standart graph representation. If the resulting graph is not “null” then the formula is satisfiable. It is not an algorithm for searching, it is a technique for symbolic manipulation.
    \end{itemize}
\end{itemize}
...
"astar(lmcut())"
    \end{verbatim}
    \item But in case of this application, There is no need to compile anything manually, as PerformanceAnalyser does it so just add fast-downward.py file in the same folder with PerformanceAnalyser.py
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{\texttt{cost\_effect\_performance\_analyser()}}: Explores how the cost values  impacts planning difficulty by systematically solving the same problem in 3 different scenarios-- no cost, generous max cost, and rescricted max cost.
\end{itemize}
...
\paragraph{Setup}
\begin{itemize}
    \item \textbf{Grid Size}: Varies from $2 \times 2$ until $20 \times 20$ or untill the timeout is reached.
...
\paragraph{Hypothesis:}
It is excepted that the specialized heuristic search algorithms of classical planners will significantly outperform the general-purpose SMT solver in both solve time and plan quality (length). The SMT approach's strength lies in expressiveness, not raw planning speed for classical domains and considering main purpose of SMT approach is checking satifiability, its performance on plan length is also expected to be poor.
\paragraph{Setup}
\begin{itemize}
